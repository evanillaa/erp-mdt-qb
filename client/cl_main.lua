...
LoadQBCoreVersion = function()
    Citizen.CreateThread(function()
        while true do
            ...
            if dist < 2 then
                DrawText3D(Config["PublicRecords"]['Coords'], "[E] View Public Records")
    ...
    OpenPublicRecords = function()
        ...
        TriggerEvent('un-mdt:animation')
        local PlayerInfo = RPC.execute("un-mdt:getInfo")
    ...
    function EnableGUI(enable)
        if enable then 
            SendNUIMessage({type = "bulletin", data = RPC.execute("un-mdt:dashboardbulletin")})
            SendNUIMessage({type = "dispatchmessages", data = RPC.execute("un-mdt:dashboardMessages")})
            SendNUIMessage({type = "getActiveUnits", lspd = RPC.execute("un-mdt:getActiveLSPD"), ems = RPC.execute("un-mdt:getActiveEMS")})
            ...
            SendNUIMessage({type = "warrants", data = RPC.execute("un-mdt:getWarrants")})
        end
        SetNuiFocus(enable, enable)
        SendNUIMessage({type = "show", enable = enable, job = QBCore.Functions.GetPlayerData().job.name, isAdmin = RPC.execute("un-mdt:isAdmin")})
        isOpen = enable
        TriggerEvent('un-mdt:animation')
    end
    function RefreshGUI()
        SetNuiFocus(false, false)
        SendNUIMessage({type = "show", enable = false, job = QBCore.Functions.GetPlayerData().job.name, isAdmin = RPC.execute("un-mdt:isAdmin")})
        isOpen = false
    end
    AddEventHandler('un-mdt:animation', function()
        if not isOpen then 
            return 
        end
    ...
    RegisterNUICallback("deleteBulletin", function(data, cb)
        local ignoreId, sentData, job = RPC.execute("un-mdt:deleteBulletin", data.id, data.time)
    ...
    RegisterNUICallback("updateLicence", function(data, cb)
        ...
        local cid = data.cid
        RPC.execute("un-mdt:updateLicence", type, status, cid)
    end)
    RegisterNUICallback("newBulletin", function(data, cb)
        local ignoreId, sentData, job = RPC.execute("un-mdt:newBulletin", data.title, data.info, data.time)
        if ignoreId == GetPlayerServerId(PlayerId()) then 
            return 
        end
        if job == 'police' and QBCore.Functions.GetPlayerData().job.name == "police" then
    ...
    RegisterNetEvent('un-mdt:open')
    AddEventHandler('un-mdt:open', function(job, jobLabel, lastname, firstname)
        open = true
        EnableGUI(open)
        local x, y, z = table.unpack(GetEntityCoords(PlayerPedId()))
        local currentStreetHash, intersectStreetHash = GetStreetNameAtCoord(x, y, z)
    ...
    RegisterNUICallback("searchProfiles", function(data, cb)
        SendNUIMessage({type = "profiles", data = RPC.execute("un-mdt:searchProfile", data.name), isLimited = RPC.execute("un-mdt:isLimited")})
    end)
    RegisterNUICallback("saveProfile", function(data, cb)
        RPC.execute("un-mdt:saveProfile", data.pfp, data.description, data.id)
    end)
    RegisterNUICallback("newWeapon", function(data, cb)
        RPC.execute("un-mdt:addWeapon", data.id, data.serialnumber)
    end)
    RegisterNUICallback("getProfileData", function(data, cb)
        local isLimited = RPC.execute("un-mdt:isLimited")
        local ProfileData = RPC.execute("un-mdt:getProfileData", data.id)
    ...
    RegisterNUICallback("newTag", function(data, cb)
        if data.id ~= "" and data.tag ~= "" then
            RPC.execute("un-mdt:newTag", data.id, data.tag)
        end
    end)
    RegisterNUICallback("removeProfileTag", function(data, cb)
        RPC.execute("un-mdt:removeProfileTag", data.cid, data.text)
    end)
    RegisterNUICallback("addGalleryImg", function(data, cb)
        RPC.execute("un-mdt:addGalleryImg", data.cid, data.URL)
    end)
    RegisterNUICallback("missingCitizen", function(data, cb)
        RPC.execute("un-mdt:missingCitizen", data.cid, data.bool)
    end)
    RegisterNUICallback("removeGalleryImg", function(data, cb)
        RPC.execute("un-mdt:removeGalleryImg", data.cid, data.URL)
    end)
    RegisterNUICallback("searchIncidents", function(data, cb)
        SendNUIMessage({type = "incidents", data = RPC.execute("un-mdt:searchIncidents", data.incident)})
    end)
    RegisterNUICallback("getIncidentData", function(data, cb)
        local sentData, sentConvictions = RPC.execute("un-mdt:getIncidentData", data.id)
    ...
    RegisterNUICallback("incidentSearchPerson", function(data, cb)
        SendNUIMessage({type = "incidentSearchPerson", data = RPC.execute("un-mdt:incidentSearchPerson", data.name)})
    end)
    RegisterNUICallback("searchBolos", function(data, cb)
        SendNUIMessage({type = "bolos", data = RPC.execute("un-mdt:searchBolos", data.searchVal)})
    end)
    RegisterNUICallback("getAllBolos", function(data, cb)
        SendNUIMessage({type = "bolos", data = RPC.execute("un-mdt:getAllBolos")})
    end)
    RegisterNUICallback("getAllMissing", function(data, cb)
        SendNUIMessage({type = "missing", data = RPC.execute("un-mdt:getAllMissing")})
    end)
    RegisterNUICallback("getAllIncidents", function(data, cb)
        SendNUIMessage({type = "incidents", data = RPC.execute("un-mdt:getAllIncidents")})
    end)
    RegisterNUICallback("getBoloData", function(data, cb)
        SendNUIMessage({type = "boloData", data = RPC.execute("un-mdt:getBoloData", data.id)})
    end)
    RegisterNUICallback("jailPlayer", function(data, cb)
        RPC.execute("un-mdt:JailPlayer", data.cid, data.time)
    end)
    RegisterNUICallback("newBolo", function(data, cb)
        local sentData = RPC.execute("un-mdt:newBolo", data.existing, data.id, data.title, data.plate, data.owner, data.individual, data.detail, data.tags, data.gallery, data.officers, data.time)
        SendNUIMessage({type = "boloComplete", data = sentData})
    end)
    RegisterNUICallback("deleteBolo", function(data, cb)
        RPC.execute("un-mdt:deleteBolo", data.id, data.time)
    end)
    RegisterNUICallback("deleteIncident", function(data, cb)
        RPC.execute("un-mdt:deleteIncident", data.id, data.time)
    end)
    RegisterNUICallback("deleteMissing", function(data, cb)
        RPC.execute("un-mdt:deleteMissing", data.id, data.time)
    end)
    RegisterNUICallback("deleteReport", function(data, cb)
        RPC.execute("un-mdt:deleteReport", data.id, data.time)
    end)
    RegisterNUICallback("deleteICU", function(data, cb)
        RPC.execute("un-mdt:deleteICU", data.id)
    end)
    RegisterNUICallback("getAllReports", function(data, cb)
        SendNUIMessage({type = "reports", data = RPC.execute("un-mdt:getAllReports")})
    end)
    RegisterNUICallback("getReportData", function(data, cb)
        SendNUIMessage({type = "reportData", data = RPC.execute("un-mdt:getReportData", data.id)})
    end)
    RegisterNUICallback("searchReports", function(data, cb)
        SendNUIMessage({type = "reports", data = RPC.execute("un-mdt:searchReports", data.name)})
    end)
    RegisterNUICallback("newReport", function(data, cb)
        local sentData = RPC.execute("un-mdt:newReport", data.existing, data.id, data.title, data.type, data.detail, data.tags, data.gallery, data.officers, data.civilians, data.time)
        SendNUIMessage({type = "reportComplete", data = sentData})
    end)
    RegisterNUICallback("searchVehicles", function(data, cb)
        local sentData = RPC.execute("un-mdt:searchVehicles", data.name)
        for i = 1, #sentData do
    ...
    RegisterNUICallback("saveVehicleInfo", function(data, cb)
        RPC.execute("un-mdt:saveVehicleInfo", data.plate, data.imageurl, data.notes)
    end)
    RegisterNUICallback("saveWeaponInfo", function(data, cb)
        RPC.execute("un-mdt:saveWeaponInfo", data.serialnumber, data.imageurl, data.brand, data.type, data.notes)
    end)
    RegisterNUICallback("saveMissingInfo", function(data, cb)
        RPC.execute("un-mdt:saveMissingInfo", data.id, data.imageurl, data.notes)
    end)
    RegisterNUICallback("knownInformation", function(data, cb)
        RPC.execute("un-mdt:knownInformation", data.type, data.status, data.plate)
    end)
    RegisterNUICallback("getAllLogs", function(data, cb)
        SendNUIMessage({type = "getAllLogs", data = RPC.execute("un-mdt:getAllLogs")})
    end)
    RegisterNUICallback("getPenalCode", function(data, cb)
        local titles, penalcode, job = RPC.execute("un-mdt:getPenalCode")
        SendNUIMessage({type = "getPenalCode", titles = titles, penalcode = penalcode, job = job})
    end)
    RegisterNUICallback("toggleDuty", function(data, cb)
        RPC.execute("un-mdt:toggleDuty", data.cid, data.status)
    end)
    RegisterNUICallback("setCallsign", function(data, cb)
        callSign = RPC.execute("un-mdt:setCallsign", data.cid, data.newcallsign)
    end)
    RegisterNUICallback("saveIncident", function(data, cb)
        RPC.execute("un-mdt:saveIncident", data.ID, data.title, data.information, data.tags, data.officers, data.civilians, data.evidence, data.associated, data.time)
    end)
    RegisterNUICallback("removeIncidentCriminal", function(data, cb)
        RPC.execute("un-mdt:removeIncidentCriminal", data.cid, data.incidentId)
    end)
    RegisterNUICallback("setWaypoint", function(data, cb)
        local callInformation = RPC.execute("un-mdt:setWaypoint", data.callid)
        SetNewWaypoint(callInformation['origin']['x'], callInformation['origin']['y'])
    end)
    RegisterNUICallback("callDetach", function(data, cb)
        local callid, sentData = RPC.execute("un-mdt:callDetach", data.callid)
    ...
    RegisterNUICallback("removeCall", function(data, cb)
        local src, callid = RPC.execute("un-mdt:removeCall", data.callid)
    ...
    RegisterNUICallback("callAttach", function(data, cb)
        local callid, sentData = RPC.execute("un-mdt:callAttach", data.callid)
        local job = QBCore.Functions.GetPlayerData().job.name
        if job == "police" or job == 'ambulance' then
    ...
    RegisterNUICallback("attachedUnits", function(data, cb)
        local sentData, callid = RPC.execute("un-mdt:attachedUnits", data.callid)
        SendNUIMessage({type = "attachedUnits", data = sentData, callid = callid})
    end)
    RegisterNUICallback("callDispatchDetach", function(data, cb)
        RPC.execute("un-mdt:callDispatchDetach", data.callid, data.cid)
    end)
    RegisterNUICallback("setDispatchWaypoint", function(data, cb)
        local callInformation = RPC.execute("un-mdt:setDispatchWaypoint", data.callid)
        SetNewWaypoint(callInformation['origin']['x'], callInformation['origin']['y'])
    end)
    RegisterNUICallback("callDragAttach", function(data, cb)
        RPC.execute("un-mdt:callDragAttach", data.callid, data.cid)
    end)
    RegisterNUICallback("setWaypointU", function(data, cb)
        local sentData = RPC.execute("un-mdt:setWaypoint:unit", data.cid)
        SetNewWaypoint(sentData.x, sentData.y) 
    end)
    RegisterNUICallback("dispatchMessage", function(data, cb)
        local sentData = RPC.execute("un-mdt:sendMessage", data.message, data.time)
        local job = QBCore.Functions.GetPlayerData().job.name
        if job == "police" or job == 'ambulance' then
    ...
    RegisterNUICallback("refreshDispatchMsgs", function(data, cb)
        SendNUIMessage({type = "dispatchmessages", data = RPC.execute("un-mdt:refreshDispatchMsgs")})
    end)
end
...
LoadESXVersion = function()
    ...
    function EnableGUI(enable)
        ...
        SetNuiFocus(enable, enable)
        SendNUIMessage({type = "show", enable = enable, job = ESX.GetPlayerData().job.name, isAdmin = RPC.execute("un-mdt:isAdmin")})
        isOpen = enable
        TriggerEvent('un-mdt:animation')
    end
    function RefreshGUI()
        SetNuiFocus(false, false)
        SendNUIMessage({type = "show", enable = false, job = ESX.GetPlayerData().job.name, isAdmin = RPC.execute("un-mdt:isAdmin")})
        isOpen = false
    end
    AddEventHandler('un-mdt:animation', function()
        if not isOpen then 
    ...
    RegisterNUICallback("deleteBulletin", function(data, cb)
        local ignoreId, sentData, job = RPC.execute("un-mdt:deleteBulletin", data.id)
        if ignoreId == GetPlayerServerId(PlayerId()) then 
            return 
        end
    ...
    RegisterNUICallback("deleteBolo", function(data, cb)
        RPC.execute("un-mdt:deleteBolo", data.id)
    end)
    RegisterNUICallback("deleteIncident", function(data, cb)
        RPC.execute("un-mdt:deleteIncident", data.id, data.time)
    end)
...
LoadNopixelVersion = function()
    ...
    function EnableGUI(enable)
        ...
        SetNuiFocus(enable, enable)
        SendNUIMessage({type = "show", enable = enable, job = exports['iconic_handler']:isPed("myjob"), isAdmin = RPC.execute("un-mdt:isAdmin")})
        isOpen = enable
        TriggerEvent('un-mdt:animation')
    end
    function RefreshGUI()
        SetNuiFocus(false, false)
        SendNUIMessage({type = "show", enable = false, job = exports['iconic_handler']:isPed("myjob"), isAdmin = RPC.execute("un-mdt:isAdmin")})
        isOpen = false
    end
    AddEventHandler('un-mdt:animation', function()
        if not isOpen then 
    ...
    RegisterNUICallback("deleteBulletin", function(data, cb)
        ...
        if job == 'police' and exports['iconic_handler']:isPed("myjob") == "police" then
            SendNUIMessage({type = "deleteBulletin", data = sentData})
        elseif job == exports['iconic_handler']:isPed("myjob") then
            SendNUIMessage({type = "deleteBulletin", data = sentData})
        end
    end)
    RegisterNUICallback("updateLicence", function(data, cb)
    ...
    AddEventHandler('un-mdt:open', function(job, jobLabel, lastname, firstname)
        ...
        SendNUIMessage({type = "data", name = "Welcome, " ..firstname..' '..lastname, location = playerStreetsLocation, fullname = firstname..' '..lastname})
    end)
    RegisterNUICallback('escape', function(data, cb)
        open = false
        EnableGUI(open)
    ...
    RegisterNUICallback("callDetach", function(data, cb)
        local callid, sentData = RPC.execute("un-mdt:callDetach", data.callid)
        local job = exports['iconic_handler']:isPed("myjob")
        if job == "police" or job == 'ambulance' then 
            SendNUIMessage({type = "callDetach", callid = callid, data = tonumber(sentData)}) 
        end
    end)
